...
export const meta = {
  ...
  description:
    'Edmonds-Karp is an implementation of Ford-Fulkerson that uses BFS to find the shortest augmenting path in the residual graph. This guarantees a polynomial time bound of O(V * E^2) regardless of edge capacities.',
  rendererType: 'graph',
  graphType: 'directed-weighted',
  pseudocode: [
    'initialize flow = 0 on all edges',
    'while BFS finds path from source to sink:',
    '  BFS on residual graph for shortest path',
...
export function generateSteps(input) {
  ...
  recorder.add('message', [], -1,
    `Starting Edmonds-Karp. Source = ${source}, Sink = ${sink}`,
  ...
  while (true) {
    ...
    recorder.add('visit', [source], 2,
      `Iteration ${iteration}: BFS from source (node ${source}) for shortest augmenting path`,
      ...
      for (let v = 0; v < numNodes; v++) {
        ...
        if (!visitedArr[v] && residual > 0) {
          ...
          recorder.add('visit', [v], 2,
            `BFS visits node ${v} from ${u} (residual capacity: ${residual})`,
    ...
    recorder.add('visit', [...pathNodes], 3,
      `Shortest augmenting path: ${pathStr}. Bottleneck = ${bottleneck}`,
  ...
  recorder.add('message', [], -1,
    `Edmonds-Karp complete. Maximum flow = ${totalFlow}`,
